...
#1、idea配置快捷键
![idea简单快捷键.png](F:\dreamStudy\dailyStudy\知识学习\日常学习记录\images\idea简单快捷键.png)
...
![rcs(本地版本控制系统).webp](F:\dreamStudy\dailyStudy\知识学习\日常学习记录\images\rcs(本地版本控制系统).webp)

##2.2、svn(集中化版本控制系统)
**原理：**在一个单一的集中管理的服务器(central server)，保存所有文件的修订版本。协同工作的人们都通过客户端连到这台服务器，取出最新的文件或者提交更新。

![svn(集中化版本控制系统).webp](F:\dreamStudy\dailyStudy\知识学习\日常学习记录\images\svn(集中化版本控制系统).webp)

##2.3、git(分布式版本控制系统)
**原理：**客户端并不只提取最新版本的文件快照，而是把代码仓库完整地镜像下来。每一次的克隆操作，实际上都是一次对代码仓库的完整备份。

![git(分布式版本控制系统).webp](F:\dreamStudy\dailyStudy\知识学习\日常学习记录\images\git(分布式版本控制系统).webp)
...
**本地结构：**
    ![git结构.png](F:\dreamStudy\dailyStudy\知识学习\日常学习记录\images\git结构.png)

工作区 git add 到 暂存区 git commit 本地库  git push 到 远程仓库 

----
**git团队内部合作：**
    ![git团队内部协作.png](F:\dreamStudy\dailyStudy\知识学习\日常学习记录\images\git团队内部协作.png)

**git跨团队合作：**
    ![git跨团队合作.png](F:\dreamStudy\dailyStudy\知识学习\日常学习记录\images\git跨团队合作.png)
...
![git版本穿梭结构.png](F:\dreamStudy\dailyStudy\知识学习\日常学习记录\images\git版本穿梭结构.png)  
注意：使用较多的都是第一种hard参数。
整体流程：将文件add到暂存区-->通过commit提交到本地库

##3.5、提交分支相关简介
![git分支的结构.png](F:\dreamStudy\dailyStudy\知识学习\日常学习记录\images\git分支的结构.png)
(1)、分支是使用多条线同时推进多个任务。
